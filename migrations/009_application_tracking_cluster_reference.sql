...
CREATE TEMP TABLE application_events_m8_backup AS
SELECT * FROM application_events;

DROP TABLE application_events;

ALTER TABLE applications RENAME TO applications_m8;
...
SELECT id, job_id, profile_id, logical_cluster_id, status, current_status,
       priority, notes, follow_up_date, cv_artifact_id, source, created_at, updated_at
FROM applications_m8;

DROP TABLE applications_m8;
...
SELECT id, application_id, from_status, to_status, event_type, reason, note, created_at
FROM application_events_m8_backup;

DROP TABLE application_events_m8_backup;
